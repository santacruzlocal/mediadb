...
@section('title')
  <title>{{ trans('main.movies') . ' - ' . trans('main.dashboard') }}</title>
@stop
        ...
        <div class="panel panel-default dashboard-panel">
          <div class="panel-heading">
            <h3 class="panel-title">
              <i class="fa fa-book"></i>  {{ trans('dash.movies in db') }}
              <a href="{{ url('movies/create') }}" class="pull-right btn btn-dash-action btn-xs">{{ trans('main.create new') }}</a>
            </h3>
          </div>
               ...
               @foreach ($latest as $k => $v)
                ...
                <div class="row">
                  ...
                  <div class="media-body col-sm-10">
                    ...
                    <a type="button" href="{{ url('movies/' . $v['id']) }}" class="btn btn-dash-action success-trans">{{ trans('main.view') }}</a>

                    <a type="button" href="{{ url('movies/' . $v['id'] . '/edit') }}" class="btn btn-dash-action warning-trans">{{ trans('main.edit') }}</a>
                  </div>
                </div>
