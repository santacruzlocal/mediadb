...
class Auth {
	...
	private function registerWithSentry()
	{
	    ...
	    {
	    	//spoof the email 
	    	$email = str_random(10) . '@temp.com';
 	
		    $this->createProfile($email);

		   	$id = User::whereEmail($email)->first()->id;
		    $this->linkProfileWithIdentifier($email);

	    	return array('twitter' => $id);
	    }
	    else
	    {
	 ...
	 * @return void
	 */
	private function linkProfileWithIdentifier($email = null)
	{	
	 ...
	 * @return User
	 */
	private function createProfile($email = null)
	{
		$user = new User;
	 ...
	 * @return Redirect
	 */
	private function loginWithSentry($user)
	{
