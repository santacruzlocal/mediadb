...
use Lib\Repository\Data\DataProviderInterface;
...
class ImdbData implements DataRepositoryInterface
{
	 ...
	 * 
	 * @var Lib\Services\Handlers\ImageHandler
	 */
	...
	public function getPlot()
	{
		...
		$written = trim(head($this->crawler->filter('div[itemprop="description"] > p > .nobr')->first()->extract(array('_text'))));
		$this->plot = trim(head($this->crawler->filter('div[itemprop="description"] > p')->first()->extract(array('_text'))));
	...
	{	
		...
		$this->poster = head($this->crawler->filter('div.image > a > img')->extract(array('src')));
	...
	public function getAllSeasons($id)
	{
		...
		$seasons = $this->crawler->filter('#titleTVSeries > .txt-block > .see-more')->extract(array('_text'));

		if ( ! empty($seasons))
		{
			//extract only numbers from string
			preg_match_all('!\d+!', head($seasons), $matches);
			
			return $this->compileAllSeasons( head($matches) );
		}
	}
