...
class TmdbData implements DataRepositoryInterface, SearchProviderInterface
{
	 ...
	 * @var string
	 */
	private $append = '&append_to_response=credits,external_ids';
	...
	public function discover(array $input)
	{
		...
		if ( ! $input['howMuch']) $input['howMuch'] = 320;
	...
	{
		//if we don't find tmdbid we'll forward the
		//request to imdb parser instead
		if ( ! $model->tmdb_id)
		{
			return App::make('Lib\Repository\Data\ImdbData')->getFullTitle($model);
		}
		...
		$title = json_decode($results[0], true);
		$images = json_decode($results[1], true);

		$this->checkStatus($title, $model->id);

		$this->raw = $title;
		$this->raw['images'] = $images['images'];
	...
	private function compileSingleTitleUrl()
	{
		...
		if ($this->titleModel->type == 'movie')
		{
			...
			$this->append = '&append_to_response=casts,trailers';
		}

		$urls[] = $this->base . "$type/" . "{$this->titleModel->tmdb_id}?" . $this->key . $this->append . "&language={$this->lang}";
		$urls[] = $this->base . "$type/" . "{$this->titleModel->tmdb_id}?" . $this->key . '&append_to_response=images';
	...
	{
		return (isset($this->raw['imdb_id']) ? $this->raw['imdb_id'] : $this->raw['external_ids']['imdb_id']);
	}
	...
	public function getFullAllSeasons(Title $series)
	{
		...
		$url = $this->compileFullAllSeasonsUrl($series->tmdb_id, $series->season);

		$response = $this->call($url);
		$seasons = json_decode($response, true);

		return $this->compileFullAllSeasons($seasons ? $seasons : array(), $series->season);
	}
	...
	private function trailers()
	{
		...
		//fetch trailer from youtube if tmdb doesnt have one
		else
		{
